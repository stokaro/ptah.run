...
export default {
  ...
  databases: {
    eyebrow: "Databases",
    h2: "Know what works on your database.",
    lead: "Ten engines. Pick yours to see what Ptah manages there, the limitation worth knowing, and which release lines are tested.",
    tabsLabel: "Database engine",
    explore: (name) => `Explore ${name} support →`,
    linesTitle: "Tested release lines",
    boundaryTitle: "Evidence boundary",
    certified: "Ptah-certified",
    legacy: "Legacy-tested",
    engines: {
      postgres: {
        role: "Primary target",
        can: "Manage tables, views, functions, extensions, custom types, roles and row-level security.",
        limitTitle: "Broadest object model.",
        limit: "Enum, domain, composite and range types, triggers, sequences and materialized views are managed here too.",
        note: "Probed against a live server in CI.",
      },
      sqlite: {
        role: "Embedded",
        can: "Schema changes, including automatic table rebuilds for alterations SQLite cannot express as ALTER TABLE.",
        limitTitle: "Rebuilds are planned, not hidden.",
        limit: "A rebuild copies the table, and the plan labels it as a SQLite table rebuild.",
        note: "Embedded library, tested in-process — no server involved.",
      },
      mysql: {
        role: "Supported",
        can: "Planning that respects MySQL DDL rules: indexes, constraints, foreign keys and the SQL MySQL actually accepts.",
        limitTitle: "Not interchangeable with MariaDB.",
        limit: "Separate dialect and capability set even though the planner family is shared.",
        note: "Probed against a live server in CI.",
      },
      mariadb: {
        role: "Supported",
        can: "MariaDB-specific DDL, indexes and constraints, planned with its own dialect rules.",
        limitTitle: "Own capability set.",
        limit: "A MariaDB server is recognized by its own version string, even behind mysql://, and planned with the MariaDB preset, never MySQL’s.",
        note: "Probed against a live server in CI.",
      },
      cockroachdb: {
        role: "PostgreSQL wire protocol",
        can: "Schema changes and row-level TTL policies, planned through the PostgreSQL path with CockroachDB rules.",
        limitTitle: "Not PostgreSQL.",
        limit: "Operations CockroachDB lacks are refused or re-spelled before SQL is emitted, never shipped as they are.",
        note: "Probed against a live server in CI.",
      },
      yugabytedb: {
        role: "PostgreSQL wire protocol",
        can: "Schema changes, roles and row-level security through the PostgreSQL path.",
        limitTitle: "Index operations differ.",
        limit: "Concurrent index creation is supported; DROP INDEX CONCURRENTLY is excluded by the YugabyteDB preset.",
        note: "Probed against a live server in CI.",
      },
      sqlserver: {
        role: "Supported subset",
        can: "T-SQL tables, filtered indexes and live catalog inspection.",
        limitTitle: "A conservative subset of T-SQL.",
        limit: "Changes outside the subset need a manual migration.",
        note: "Probed against a live server in CI.",
      },
      oracle: {
        role: "Supported",
        can: "Schema inspection, planning and versioned migrations.",
        limitTitle: "Declarative convergence has known limits.",
        limit: "Comparing a declared file with a live Oracle catalog does not converge yet, and migration runs are not serialized by an advisory lock.",
        note: "Probed against a live server in CI.",
      },
      clickhouse: {
        role: "Analytical",
        can: "MergeTree tables with engine settings and sorting keys, materialized views and TTL policies.",
        limitTitle: "Capability-gated.",
        limit: "No foreign keys or transactions to model; the plan works with what the engine offers.",
        note: "Probed against a live server in CI.",
      },
      spanner: {
        role: "PostgreSQL interface",
        can: "Schema workflows through the PostgreSQL interface, tested against the Spanner emulator.",
        limitTitle: "Evidence boundary.",
        limit: "Emulator-tested; managed-service compatibility is not certified.",
        note: "The emulator does not prove the managed service.",
        boundary: [
          { v: "emulator", s: "Tested in CI" },
          { v: "managed", s: "Not certified" },
        ],
      },
    },
    capsTitle: "Check your server’s capabilities.",
    capsP: "Reports the detected version, its support level and the capability profile Ptah will plan with.",
    copy: "Copy",
    copyName: "capabilities command",
    capsHref: "https://docs.ptah.run/edge/reference/native-commands/#reading-a-servers-capability-profile",
    capsLink: "How to read the output →",
    ciTitle: "Compatibility checks, backed by CI.",
    ciP: (certified) => [
      "Automated probes run on every push to master and nightly: ",
      { strong: String(certified) },
      " Ptah-certified release lines — the lines Ptah tests in CI and commits to within their probed capabilities. Embedded SQLite and the Spanner emulator are declared as such, not as live servers.",
    ],
    ciLink: "View support evidence →",
    toolingLabel: "Use Ptah in your workflow",
    tooling: [
      { code: "ptah" },
      " on the command line. ",
      { code: "ptah-ls" },
      " in the editor for Go annotations. ",
      { href: "https://github.com/stokaro/ptah-action", text: "GitHub Action" },
      " in CI, ",
      { href: "https://operator.ptah.run/", text: "Kubernetes Operator" },
      " in the cluster, Go packages at ",
      { code: "ptah.run" },
      " in your own code.",
    ],
    ...
    atlas: [
      { code: "ptah-compat" },
      " accepts Atlas-shaped commands and runs Ptah behavior behind them. Independent clean-room implementation, not affiliated with Ariga. ",
