...
(function () {
  ...
  function setupDemo(demo) {
    ...
    // The hero counts lines instead of drawing a rule: "12 / 49" says how far
    // through a session is in the unit a reader is reading it in.
    var counter = $("[data-demo-count]", demo);
    ...
    // How many lines the settled session has, counted the way settle() lays
    // them out: a note that follows output gets a blank row in front of it.
    function lineTotal(script) {
      var n = 0;
      var blank = true;
      for (var i = 0; i < script.length; i++) {
        var kind = script[i][0];
        if (kind === "wait" || kind === "sync") continue;
        if (kind === "note" && n && !blank) n++;
        n++;
        blank = kind === "blank";
      }
      return n;
    }
    var total = lineTotal(SCRIPT);
    ...
    function paint() {
      ...
      if (counter) {
        var shown = lines.length + (typing !== null ? 1 : 0);
        counter.textContent = Math.min(shown, total) + " / " + total;
      }
    }
    ...
    if (modal) {
      ...
      // Whatever else closes it -- a browser affordance, a form -- still has
      // to leave the button label and the moved node in a consistent state.
      // The event is delivered after the dialog closed, so a reader who
      // reopened it in between -- a link to another run, a second tile --
      // would have the new overlay closed by the old event. Only a dialog
      // that is still closed is handled here.
      modal.addEventListener("close", function () {
        if (!modal.open && demo.parentNode === modal) setOpen(false);
      });
    }
    ...
    // The hero's "Try it" takes the frame over; the walkthrough holds where
    // it was and carries on when the reader comes back to it.
    demo.addEventListener("demo:hold", function () {
      if (!playing || paused) return;
      paused = true;
      suspend();
      freezeProgress();
    ...
    demo.addEventListener("demo:carry-on", function () {
      if (!playing || !paused || !autoplays()) return;
      paused = false;
      paint();
      label();
      resume();
    ...
    // /in-practice/#run-<key> opens that run, so a link from elsewhere lands on
    // the session it named rather than on the grid.
    function openFromHash() {
      var wanted = location.hash.match(/^#run-([A-Za-z]+)$/);
      if (!wanted || !tiles.length) return;
      var tile = $('[data-demo-tile][data-demo-scenario="' + wanted[1] + '"]');
      if (!tile) return;
      // Already open on another run: show the one the address names.
      if (demo.classList.contains("is-open")) return choose(wanted[1]);
